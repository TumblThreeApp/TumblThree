...
def test_state_dir_respects_xdg_state_home(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_config_dir_linux_default(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_dedup_db_is_under_data_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg-data"))
